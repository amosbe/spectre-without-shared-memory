...
__always_inline uint32_t memaccesstime(void *v) {
  uint32_t rv;
  asm volatile (
      "mfence\n"
      "lfence\n"
...
__always_inline void clflush(void *v) {
  asm volatile ("clflush 0(%0)": : "r" (v):);
}

__always_inline uint32_t rdtscp() {
  uint32_t rv;
...
__always_inline void mfence() {
  asm volatile("mfence");
}


__always_inline void walk(void *p, int count) {
  if (p == NULL)
    return;
  asm volatile(
    "movq %0, %%rsi\n"
...
__always_inline void cpuid(union cpuid *c) {
